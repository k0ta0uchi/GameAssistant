...
3. **応答生成ルール**
   ...
   - **重要: Web検索ツールの使用について**
     - ユーザーから「検索して」「調べて」「最新の情報を教えて」といった明示的な依頼があった場合のみ、Web検索ツールを使用してください。
...
pub const MEMORY_SUMMARIZE_PROMPT: &str = r#"ユーザーの発言から重要な情報を抽出し、客観的な事実として記録してください。
...
pub const PROMPT_DEFINITIONS: &[PromptDef] = &[
    PromptDef {
        ...
        default: SYSTEM_INSTRUCTION_CHARACTER,
    },
    PromptDef {
        ...
        default: AUTO_COMMENTARY_PROMPT,
    },
    PromptDef {
        ...
        default: BLOG_WRITER_SYSTEM_PROMPT,
    },
    PromptDef {
        ...
        category: "Memory",
        icon: "Brain",
        description: "ユーザー発話から重要な事実を抽出して LanceDB に記録するためのプロンプトです。※ {text} プレースホルダーを含めてください。",
        default: MEMORY_SUMMARIZE_PROMPT,
    },
    PromptDef {
        ...
        default: SESSION_SUMMARIZE_PROMPT,
    },
    PromptDef {
        ...
        default: TTS_STYLE_INSTRUCTION,
    },
];

/// 指定されたキーのプロンプトを settings.json またはデフォルト値から取得
pub fn get_prompt(root_dir: &Path, key: &str) -> String {
    let settings = crate::settings::load_settings_file(root_dir);

    // 1. settings.json のトップレベル "system_prompt" や各キーをチェック
    if key == "system_instruction_character" {
        if let Some(val) = settings.get("system_prompt").and_then(|v| v.as_str()) {
    ...
    if let Some(val) = settings.get(key).and_then(|v| v.as_str()) {
        if !val.trim().is_empty() {
            return val.to_string();
        }
    }

    // 2. settings.json の "prompts" オブジェクト内をチェック
    if let Some(prompts_obj) = settings.get("prompts").and_then(|v| v.as_object()) {
        if let Some(val) = prompts_obj.get(key).and_then(|v| v.as_str()) {
            if !val.trim().is_empty() {
                return val.to_string();
            }
        }
    ...
    for def in PROMPT_DEFINITIONS {
        if def.id == key {
            return def.default.trim().to_string();
        }
    }
...
pub fn get_all_prompts(root_dir: &Path) -> Vec<PromptItem> {
    ...
    for def in PROMPT_DEFINITIONS {
        let current_val = get_prompt(root_dir, def.id);
        let is_mod = current_val.trim() != def.default.trim();
        items.push(PromptItem {
            ...
            default: def.default.trim().to_string(),
            value: current_val,
            is_modified: is_mod,
        });
    }
...
/// プロンプトを settings.json に保存
pub fn save_prompt_value(root_dir: &Path, id: &str, value: &str) -> Result<(), String> {
    let mut current_settings = crate::settings::load_settings_file(root_dir);
    ...
    let pretty_str = serde_json::to_string_pretty(&current_settings).map_err(|e| e.to_string())?;
    std::fs::write(&settings_path, pretty_str).map_err(|e| e.to_string())?;
    Ok(())
}

/// プロンプトをデフォルトにリセット
pub fn reset_prompt_value(root_dir: &Path, id: &str) -> Result<(), String> {
    for def in PROMPT_DEFINITIONS {
        if def.id == id {
            return save_prompt_value(root_dir, id, def.default.trim());
        }
    }
